...
(defconst solarized-gray-description
  "Color theme by Ethan Schoonover, created 2011-03-24.
Ported to Emacs by Greg Pfeil, http://ethanschoonover.com/solarized.")

(defcustom solarized-gray-termcolors 16
...
(defcustom solarized-gray-degrade nil
...
(defcustom solarized-gray-broken-srgb
...
the \"Gen RGB\" column in solarized-gray-definitions.el to improve them further."
  :type 'boolean
  :group 'solarized)
...
(defvar solarized-gray-colors           ; ANSI(Solarized terminal)
...
(defun solarized-gray-face-for-index (facespec index &optional light)
  "Creates a face from facespec where the colors use the names from
  `solarized-gray-colors`."
      ...
      (let ((color-name (plist-get new-fontspec property)))
        (when color-name
          ...
          (plist-put new-fontspec
                     property
                     (nth index (assoc color-name solarized-gray-colors))))))
    (when (plist-get new-fontspec :box)
      (plist-put new-fontspec
                 :box
                 (solarized-gray-face-for-index (plist-get new-fontspec :box) index
                                           light)))
    new-fontspec))

(defun create-face-spec (name facespec)
  `(,name ((((background dark) (type graphic))
            ,(solarized-gray-face-for-index facespec
                                       (cond (solarized-gray-degrade     3)
                                             (solarized-gray-broken-srgb 2)
                                             (t                     1))))
           (((background dark) (type tty) (min-colors 256))
            ,(solarized-gray-face-for-index facespec
                                       (if (= solarized-gray-termcolors 16) 4 3)))
           (((background dark) (type tty) (min-colors  16))
            ,(solarized-gray-face-for-index facespec 4))
           (((background dark) (type tty) (min-colors   8))
            ,(solarized-gray-face-for-index facespec 5))
           ...
           (((background light) (type tty) (min-colors  16))
            ,(solarized-gray-face-for-index facespec 4 t))
           (((background light) (type tty) (min-colors   8))
            ,(solarized-gray-face-for-index facespec 5 t)))))

(defun solarized-gray-color-definitions ()
  (let ((bold        (if solarized-gray-bold 'bold        'unspecified))
        (bright-bold (if solarized-gray-bold 'unspecified 'bold))
        (underline   (if solarized-gray-underline t 'unspecified))
        (opt-under   'unspecified)
        (italic      (if solarized-gray-italic 'italic 'unspecified)))
    (cond ((eq 'high solarized-gray-contrast)
           (let ((orig-base3 base3))
             (rotatef base01 base00 base0 base1 base2 base3)
             (setf base3 orig-base3)))
          ((eq 'low solarized-gray-contrast)
           (setf back      base02
                 opt-under t)))
    (let ((bg-back   '(:background back))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                ...
                ,@(cl-case solarized-gray-diff-mode
                    (high
...
(defmacro create-solarized-gray-theme (name description color-definitions)
  `(progn
...
(provide 'solarized-gray-definitions)
